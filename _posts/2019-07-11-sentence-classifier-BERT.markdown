...
* **Classifier Results vs Human Error**: The upper bound on performance achievable of the classifiers is the level of human error in the generated labels. The inter-annotator agreements between the 10 annotators range between 0.66-0.86, so the results of the BERT classifier are within this range, with the BERT classifier on the full excerpts close to the top of this range. This indicates that actually given the difficulty of the task, the performance of these classifiers is not bad. 
